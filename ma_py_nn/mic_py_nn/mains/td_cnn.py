...
def main():
    ...
    model = TDCNN(config)
    model.build_model(enable_rnn=False, s_fr_pr=no_context_pred)
    model.init_saver()
    ...
    #################################################################
    # 4 - Create batchers
    train_data = MADBatcher(train_spk_files, train_noise_files, config, mel_feats=True, no_context_pred=no_context_pred,
                            one_hot_mask=True, simple_mix=True)
    valid_data = MADBatcher(valid_spk_files, valid_noise_files, config, mel_feats=True, no_context_pred=no_context_pred,
                            one_hot_mask=True, simple_mix=True)
    ...
    trainer = CRNNMaskTrainer(sess, model, train_data, valid_data, config, logger, mel_feats=True, use_phase=use_phase,
    ...
    #################################################################
    # 7 - Predict result
    predictor = CRNNMaskPredict(sess, model, config, mel_feats=True, use_phase=use_phase,
