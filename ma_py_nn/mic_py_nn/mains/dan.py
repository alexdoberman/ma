...
from mic_py_nn.models.dan_model import DANModel
from mic_py_nn.trainers.clustering_train import DANTrainer
from mic_py_nn.trainers.clustering_predict import DANPredict
from mic_py_nn.data_generator.batchers.dan_batcher import DANBatcher
...
def main():
    ...
    #################################################################
    # 5 - Create batchers
    train_data = DANBatcher(train_spk_files, train_noise_files, config)
    valid_data = DANBatcher(valid_spk_files, valid_noise_files, config)

    #################################################################
    # 6 - Train model
    if args.stage == 'train':
        logger  = Logger(sess, config)
        trainer = DANTrainer(sess, model, train_data, valid_data, config, logger)
        trainer.train()

    #################################################################
    # 7 - Predict result
    predictor = DANPredict(sess, model, config)
