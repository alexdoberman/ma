...
from mic_py_nn.models.dcw_model import DCWModel
from mic_py_nn.trainers.clustering_train import DCTrainer
from mic_py_nn.trainers.clustering_predict import DCPredict
...
def main():
    ...
    #################################################################
    # 5 - Create batchers
    train_data = DCBatcher(train_spk_files, train_noise_files, config)
    valid_data = DCBatcher(valid_spk_files, valid_noise_files, config)
    ...
    #################################################################
    # 7 - Predict result
    predictor = DCPredict(sess, model, config)
