...
from mic_py_nn.data_generator.batchers.multi_fft_batcher import MaskBatcherMultiFFT

from mic_py_nn.models.refine_net_model import RefineNet
from mic_py_nn.trainers.refine_net_trainer import RefineNetTrainer
from  mic_py_nn.trainers.refine_net_predict import RefineNetPredict
...
def main():
    ...
    #################################################################
    # 4 - Create batchers
    train_data = MaskBatcherMultiFFT(train_spk_files, train_noise_files, config)
    valid_data = MaskBatcherMultiFFT(valid_spk_files, valid_noise_files, config)

    #################################################################
    # 4 - Train model
    logger  = Logger(sess, config)
    trainer = RefineNetTrainer(sess, model, train_data, valid_data, config, logger)
    trainer.train()
    ...
    predictor = RefineNetPredict(sess, model, config)
