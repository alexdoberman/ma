...
from mic_py_nn.models.dcce_model import DCCEModel
from mic_py_nn.data_generator.batchers.dcce_batcher import DCCEBatcher
from mic_py_nn.trainers.clustering_train import DCCETrainer
from mic_py_nn.trainers.clustering_predict import DCCEPredict
...
def main():
    ...
    #################################################################
    # 5 - Create batchers
    train_data = DCCEBatcher(train_spk_files, train_noise_files, config)
    valid_data = DCCEBatcher(valid_spk_files, valid_noise_files, config)

    #################################################################
    # 6 - Train model
    if args.stage == 'train':
        logger  = Logger(sess, config)
        trainer = DCCETrainer(sess, model, train_data, valid_data, config, logger)
        trainer.train()

    #################################################################
    # 7 - Predict result
    predictor = DCCEPredict(sess, model, config)
