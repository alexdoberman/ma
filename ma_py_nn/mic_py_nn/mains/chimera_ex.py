...
from mic_py_nn.models.chimera_ex_model import ChimeraExModel
from mic_py_nn.data_generator.batchers.chimera_batcher import ChimeraBatcher
from mic_py_nn.trainers.clustering_train import ChimeraTrainer
...
def main():
    ...
    #################################################################
    # Let's make randomization predictable
    ...
    #################################################################
    # 5 - Create batchers
    train_data = ChimeraBatcher(train_spk_files, train_noise_files, config)
    valid_data = ChimeraBatcher(valid_spk_files, valid_noise_files, config)

    #################################################################
    # 6 - Train model
    if args.stage == 'train':
        logger  = Logger(sess, config)
        trainer = ChimeraTrainer(sess, model, train_data, valid_data, config, logger)
        trainer.train()

    #################################################################
    # 7 - Predict result
    if config.predictor.type == 'mi':
        predictor = ChimeraMaskInferencePredict(sess, model, config)
    elif config.predictor.type == 'dc':
        predictor = ChimeraClusteringPredict(sess, model, config)
