...
class DCModel(BaseModel):
    ...
    def __init__(self, config):
        super(DCModel, self).__init__(config)

        self.F               = config.model.F
        self.F_out = config.model.F_out
    ...
    def build_model(self):
        ...
        with tf.device(self.device):
            ...
            self.y = tf.placeholder("float", [None, None, self.F_out, None])
    ...
    def network(self):
        ...
        feedforward = tf_utils.conv1d_layer(BLSTM,
                                            [1, self.layer_size, self.embedding_size*self.F_out])

        # Reshape the feedforward output to have shape (T,F,K)
        embedding = tf.reshape(feedforward,
                               [shape[0], shape[1], self.F_out, self.embedding_size])
    ...
    def cost(self):
        ...
        VTV = tf.matmul(tf.transpose(V, perm=[0, 2, 1]), V)
        YTY = tf.matmul(tf.transpose(Y, perm=[0, 2, 1]), Y)
        VTY = tf.matmul(tf.transpose(V, perm=[0, 2, 1]), Y)
