...
class ChimeraExModel(BaseModel):
    """
    ...
    def __init__(self, config):
        super(ChimeraExModel, self).__init__(config)
    ...
    def network(self):
        ...
        BNET = self.X
        ########################################################
        if (self.config.model.net_type_ex == 0):
            # BGRU net
            print ('ChimeraExModel: construct BGRU net')
            for l_num in range(self.config.model.layers_count):
                BNET = tf_utils.BGRU_(BNET, self.layer_size, 'BGRU_{}'.format(l_num),
                                      activation=self.nonlinearity)
        elif(self.config.model.net_type_ex == 1):
            # BLSTM net
            print ('ChimeraExModel: construct BLSTM net')
            for l_num in range(self.config.model.layers_count):
                BNET = tf_utils.BLSTM_(BNET, self.layer_size, 'BLSTM_{}'.format(l_num),
                                       activation=self.nonlinearity)
        elif (self.config.model.net_type_ex == 2):
            # BGridLSTM_ net
            print ('ChimeraExModel: construct BGridLSTM net')
            for l_num in range(self.config.model.layers_count):
                BNET = tf_utils.BGridLSTM_(BNET, self.layer_size, 'BLSTM_{}'.format(l_num),
                                       activation=self.nonlinearity)
        else:
            raise Exception("ERROR: ChimeraExModel, unsupported net_type_ex {}".format(self.config.model.net_type_ex))

        ########################################################

        # Feedforward layer
        feedforward = tf_utils.conv1d_layer(BNET,
