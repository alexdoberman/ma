...
class BaseSAD:
    ...
    def get_istft(self, arr):
        sig = istft(arr, overlap=self.mic_config.overlap)
        return sig
    ...
    def do_sad(self, in_wav_path, filter_cfg, **kwargs):
        """
        You must return frame based segmentation where there is a speech absence detect

        :param in_wav_path: - path to mic wav
        :param filter_cfg:
        :param kwargs:
        :return:
            np.array - shape (frame_count)

            Example: np.array([0,0,1,1,0])
            where 1 - speech absent frame
        """
        pass
